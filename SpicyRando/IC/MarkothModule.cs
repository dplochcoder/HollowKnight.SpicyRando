    ...
    protected override void ModifyGhostWarrior(PlayMakerFSM fsm, Wrapped<int> baseHp)
    {
        ...
        shieldAttackFsm
            ...
            .AddFirstAction(
                ...
                {
                    if (!rage2.Value || rage2Shield.Value)
                        return;
                    rage2Shield.Value = true;

                    var summonShieldFsm = shieldAttackFsm
                        .FsmVariables.GetFsmGameObject("Shield 1")
                        .Value.LocateMyFSM("Summon Shield");
                    ...
                    summonState2.AddLastAction(
                        new Lambda(() =>
                        {
                            CopyITween(
                                shieldControlFsm.GetState("Tween Out"),
                                shieldControlFsm.FsmVariables.GetFsmGameObject("Shield 2")
                            );
                            CopyITween(
                                shieldControlFsm.GetState("Tween In"),
                                shieldControlFsm.FsmVariables.GetFsmGameObject("Shield 2")
                            );
                        })
                    );
                    ...
                    shieldAttackFsm
                        .GetState("Send Summon")
                        .GetFirstActionOfType<SendEventByName>()
                        .sendEvent.Value = "SUMMON SHIELD 2";
        ...
        waitState.AddFirstAction(
            new Lambda(() =>
            {
                if (UpdatePhase(fsm, baseHp, phase2, 0.7f))
                {
                    ...
                    shieldTemplate.Value = Object.Instantiate(
                        shieldAttackFsm
                            .FsmVariables.GetFsmGameObject("Shield 1")
                            .Value.LocateMyFSM("Summon Shield")
                            .FsmVariables.GetFsmGameObject("Shield 2")
                            .Value
                    );
...
internal class MarkothFeature : AbstractGhostWarriorFeature<MarkothModule>
{
    ...
    public override void ApplyLogicChanges(GenerationSettings gs, LogicManagerBuilder lmb) =>
        lmb.DoMacroEdit(
            new(
                "COMBAT[Markoth]",
                "ORIG + (SPICYCOMBATSKIPS | MASKSHARDS>19 + FULLDASH + (FIREBALL>1 | SCREAM>1 | QUAKE>1))"
            )
        );
}
