...
internal class NoEyesModule : AbstractGhostWarriorModule
{
    ...
    protected override void ModifyGhostWarrior(PlayMakerFSM fsm, Wrapped<int> baseHp)
    {
        ...
        static void FixSpawnShot(GameObject obj)
        {
            ...
            if (control.ActiveStateName == "Travel")
        ...
        shotSpawnFsm
            ...
            .InsertAction(
                new Lambda(() =>
                    FixSpawnShot(shotSpawnFsm.FsmVariables.GetFsmGameObject("Shot").Value)
                ),
                4
            );
        shotSpawnFsm
            ...
            .AddFirstAction(
                new Lambda(() =>
                {
                    if (UpdatePhase(fsm, baseHp, noEscape, 0.5f))
                    {
                        shotSpawnFsm
                            .GetState("Spawn L")
                            .GetActionsOfType<RandomFloat>()[1]
                            .min.Value = -1f;
                        shotSpawnFsm
                            .GetState("Spawn R")
                            .GetActionsOfType<RandomFloat>()[1]
                            ...
                            .AddFirstAction(
                                new Lambda(() =>
                                {
                                    for (int i = 0; i < 6; i++)
                                    {
                                        ...
                                        spawn
                                            .LocateMyFSM("Control")
                                            .FsmVariables.GetFsmFloat("Speed")
                                            .Value = speed;
                                        FixSpawnShot(spawn);
                                    }
                                })
                    ...
                    float pct =
                        1 - (fsm.gameObject.GetComponent<HealthManager>().hp * 1f / baseHp.Value);

                    float warpProb = 0.35f + pct * 0.4f;
                    fsm.GetState("Decide").GetFirstActionOfType<SendRandomEvent>().weights =
                    [
                        1 - warpProb,
                        warpProb,
                    ];
                    ...
                    var warpWait = fsm
                        ...
                        .GetState("Hover")
                        .GetFirstActionOfType<WaitRandom>();
                    warpWait.timeMin = 3.5f - pct * 2.25f;
                    warpWait.timeMax = 5f - pct * 3f;

                    shotSpawnFsm.FsmVariables.GetFsmFloat("Spawn Pause").Value =
                        2.25f - 1.75f * Mathf.Pow(pct, 0.65f);
                })
            );
    }
}
...
internal class NoEyesFeature : AbstractGhostWarriorFeature<NoEyesModule>
{
    ...
    public override void ApplyLogicChanges(GenerationSettings gs, LogicManagerBuilder lmb) =>
        lmb.DoMacroEdit(
            new(
                "COMBAT[No_Eyes]",
                "ORIG + (SPICYCOMBATSKIPS | MASKSHARDS>15 + (FIREBALL | SCREAM) + (ANYCLAW | WINGS))"
            )
        );
}
