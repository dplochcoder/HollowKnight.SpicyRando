...
internal class GorbModule : AbstractGhostWarriorModule
{
    ...
    private void SpawnSpears(
        ...
        int count,
        float speedup,
        AudioPlayerOneShotSingle audio,
        float pitch
    )
    {
    ...
    protected override void ModifyGhostWarrior(PlayMakerFSM fsm, Wrapped<int> baseHp)
    {
        ...
        void SetMoveSpeeds(float speed, float accel, float warpChance, float warpDelay)
        {
            ...
            warpState.GetFirstActionOfType<SendRandomEvent>().weights =
            [
                1 - warpChance,
                warpChance,
            ];
        ...
        attackState.GetFirstActionOfType<Tk2dPlayAnimationWithEvents>().animationCompleteEvent =
                    ...
                    IEnumerator Attack()
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                for (int i = 0; i < len; i++)
                                    offs.Add(Offset(count, len) * 2 * i * sign);
                            }
...
internal class GorbFeature : AbstractGhostWarriorFeature<GorbModule>
{
    ...
    public override void ApplyLogicChanges(GenerationSettings gs, LogicManagerBuilder lmb) =>
        lmb.DoMacroEdit(
            new(
                "COMBAT[Gorb]",
                "ORIG + ANYDASH + (SPICYCOMBATSKIPS | MASKSHARDS>15 + (FIREBALL | SCREAM))"
            )
        );
}
