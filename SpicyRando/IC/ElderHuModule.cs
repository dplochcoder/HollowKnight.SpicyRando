...
internal class ElderHuModule : AbstractGhostWarriorModule
{
    ...
    private IEnumerable<PlayMakerFSM> GetAllRings(PlayMakerFSM fsm)
    {
        for (int i = 1; i <= 17; i++)
            yield return fsm
                .FsmVariables.GetFsmGameObject($"Ring {i}")
                .Value.LocateMyFSM("Control");
    }

    private void SetSpeed(PlayMakerFSM fsm, float speed, float ringDelay, float ringSpeedup)
    {
        fsm
            .gameObject.LocateMyFSM("Movement")
            .GetState("Hover")
            .GetFirstActionOfType<ChaseObject>()
            .speedMax = speed;
    ...
    private void PlayMegaRingSounds(PlayMakerFSM fsm, float delay)
    {
        fsm.gameObject.DoAfter(
            ...
            {
                foreach (
                    var action in fsm.GetState("M 1").GetActionsOfType<AudioPlayerOneShotSingle>()
                )
                    action.DoPlayRandomClip();
            },
            delay
        );
    }
    ...
    protected override void ModifyGhostWarrior(PlayMakerFSM fsm, Wrapped<int> baseHP)
    {
        ...
        fsm.GetState("Place Rings")
            .AddLastAction(
                new Lambda(() =>
                {
                    ...
                    {
                        if (
                            (phase3.Value && Random.Range(0, 4) == 0)
                            || (phase2.Value && Random.Range(0, 2) == 0)
                            || (!phase2.Value && !phase3.Value)
                        )
                        {
                            int a = phase3.Value
                                ? 2
                            ...
                            ExecuteSpecialAttack(fsm, didShadeDodge);
        ...
        megaState.AddLastAction(
            new Lambda(() =>
            {
                List<PlayMakerFSM> controls = [.. GetAllRings(fsm)];

                if (alts.Value.Count == 0)
                    alts.Value =
                    [
                        .. controls.Select(ringFsm =>
                            Object.Instantiate(ringFsm.gameObject).LocateMyFSM("Control")
                        ),
                    ];
                for (int i = 0; i < 17; i++)
                    alts.Value[i].transform.position = controls[i].transform.position;
                    ...
                    IEnumerator Standard()
                    {
                        for (int i = 0; i < 9; i++)
                        {
                            ...
                            yield return new WaitForSeconds(
                                i == 8
                                    ? (phase3.Value ? 0.65f : (phase2.Value ? 0.75f : 1f))
                                    : (phase3.Value ? 0.075f : (phase2.Value ? 0.15f : 0.3f))
                            );
                        }
                ...
                else
                {
                    ...
                    IEnumerator DubStep()
                    {
                        ...
                        for (int j = 0; j < 5; j++)
                        {
                            ...
                            for (int i = 0; i < 17; i++)
                                if ((i + j) % 2 == off)
                                    src[i].gameObject.SetActive(true);
                            PlayMegaRingSounds(fsm, 0.3f);
...
internal class ElderHuFeature : AbstractGhostWarriorFeature<ElderHuModule>
{
    ...
    public override void ApplyLogicChanges(GenerationSettings gs, LogicManagerBuilder lmb) =>
        lmb.DoMacroEdit(
            new(
                "COMBAT[Elder_Hu]",
                "ORIG + FULLSHADOWDASH + ((SPICYCOMBATSKIPS + (UPSLASH | SIDESLASH)) | MASKSHARDS>15 + (FIREBALL | SCREAM))"
            )
        );
}
