...
internal class SuperMylaController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (
            knight!.transform.position.y < 9.5f
            && knight.transform.position.x > 25
            && myla!.GetComponent<HealthManager>().hp < SuperMylaModule.MYLA_HEALTH
        )
        {
            locked = true;
...
internal class SuperMylaModule : ItemChanger.Modules.Module
{
    public override void Initialize() =>
        Events.AddSceneChangeEdit("Crossroads_45", MakeMylaImmortal);

    public override void Unload() =>
        Events.RemoveSceneChangeEdit("Crossroads_45", MakeMylaImmortal);
    ...
    private void MakeMylaImmortal(Scene scene)
    {
        ...
        cooldown.AddFirstAction(
            new Lambda(() =>
            {
                if (spawnBulletChain)
                    fsm.SetState("Attack Antic");
                for (int i = 1; i < cooldown.Actions.Length; i++)
                    cooldown.Actions[i].Enabled = !spawnBulletChain;
            })
        );
    }
