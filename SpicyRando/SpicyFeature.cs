...
internal abstract class AbstractSpicyFeature<T> : SpicyFeature
    where T : Module, new()
{
    ...
    {
        if (value)
            settings.Enabled.Add(Name);
        else
            settings.Enabled.Remove(Name);
    }
...
internal static class SpicyFeatures
{
    ...
    private static Dictionary<string, List<SpicyFeature>> CreateCategories(
        IEnumerable<SpicyFeature> features
    )
    {
        Dictionary<string, List<SpicyFeature>> ret = [];
        foreach (var feature in features)
        {
            if (feature.CategoryName == null)
                continue;
    ...
    private static readonly Dictionary<string, List<SpicyFeature>> CATEGORIES = CreateCategories(
        ALL_FEATURES
    );

    internal static IEnumerable<SpicyFeature> Category(string name) =>
        CATEGORIES.GetOrDefault(name, () => []);
}
